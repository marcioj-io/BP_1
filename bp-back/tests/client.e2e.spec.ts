...
import {
  generateCreateInput,
  generateUpdateInput,
} from './fixtures/client/client.fixture.generator';
import { revertClientToInitialState } from './fixtures/client/client.test-utils';
...
describe('Client module E2E Tests', () => {
  ...
  beforeEach(async () => {
    if (clientId) {
      await revertClientToInitialState(prisma, clientId);
    }
  });

  describe('Create Client', () => {
    const endPointUrl = '/client';
  ...
  describe('Get Client by id', () => {
    ...
    it('Should throw error if Client id doesnt exist', async () => {
  ...
  describe('Delete Client', () => {
    ...
    it('Should inactive a Client', async () => {
      await request(app.getHttpServer())
        .delete(`${endPointUrl}/${clientId}`)
        .query({ version: 1 })
        .set('Authorization', `Bearer ${bearerToken}`)
        .expect(200);
    });
