...
import {
  generateCreateInput,
  generateUpdateInput,
} from './fixtures/source/source.fixture.generator';
import { revertSourceToInitialState } from './fixtures/source/source.test-utils';
import { authenticateUser } from './shared/auth.shared';
import { baseAuthenticationTests } from './shared/base.tests';

describe('Source module E2E Tests', () => {
  ...
  beforeEach(async () => {
    if (sourceId) {
      await revertSourceToInitialState(prisma, sourceId);
    }
  });

  describe('Create Source', () => {
    const endPointUrl = '/source';
  ...
  describe('Get Source by id', () => {
    ...
    it('Should throw error if Source id doesnt exist', async () => {
  ...
  describe('Delete Source', () => {
    ...
    it('Should inactive a Source', async () => {
      await request(app.getHttpServer())
        .delete(`${endPointUrl}/${sourceId}`)
        .query({ version: 1 })
        .set('Authorization', `Bearer ${bearerToken}`)
        .expect(200);
    });
