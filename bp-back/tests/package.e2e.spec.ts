...
import {
  generateCreateInput,
  generateUpdateInput,
} from './fixtures/package/package.fixture.generator';
import { revertPackageToInitialState } from './fixtures/package/package.test-utils';
import { authenticateUser } from './shared/auth.shared';
import { baseAuthenticationTests } from './shared/base.tests';

describe('Package module E2E Tests', () => {
  ...
  beforeEach(async () => {
    if (packageId) {
      await revertPackageToInitialState(prisma, packageId);
    }
  });

  describe('Create Package', () => {
    const endPointUrl = '/package';
  ...
  describe('Get Package by id', () => {
    ...
    it('Should throw error if Package id doesnt exist', async () => {
  ...
  describe('Delete Package', () => {
    ...
    it('Should inactive a Package', async () => {
      await request(app.getHttpServer())
        .delete(`${endPointUrl}/${packageId}`)
        .query({ version: 1 })
        .set('Authorization', `Bearer ${bearerToken}`)
        .expect(200);
    });
